...
## HTML-проверка Dreamkas вместо JSON

В v6.34 добавлена обработка ситуации, когда `kabinet.dreamkas.ru/api` вместо JSON API возвращает HTML-страницу с JavaScript-проверкой.

Признак в логе:

```text
RESPONSE 200 GET https://kabinet.dreamkas.ru/api/shops
{
  "raw": "<html>..."
}
```
...
```text
1. Распознаёт HTML challenge Dreamkas
2. Считывает cookie __js_p_
3. Вычисляет __jhash_
4. Устанавливает cookies __jhash_ и __jua_
5. Повторяет API-запрос
```

Если автоматическая обработка не помогла, откройте `https://kabinet.dreamkas.ru` в браузере с этого же компьютера и повторите запуск программы через 1-2 минуты.
...
## Исправление передачи телефона покупателя

В v6.34 исправлена передача контакта покупателя в Dreamkas.

Теперь:

```text
Email покупателя   → attributes.email
Телефон покупателя → attributes.phone
```

Раньше при выборе телефона номер мог уходить в поле `email`, из-за чего в ОФД он отображался как:

```text
Эл. адрес покупателя: +7987...
```

После исправления телефон передаётся отдельным полем `phone`.
...
```text
0. По умолчанию на кассе / общая система — DEFAULT
1. Упрощённая система — доходы — УСН доходы
2. Упрощённая система — доходы минус расходы — УСН доходы-расходы
3. Единый сельскохозяйственный налог — ЕСХН
4. Патентная система налогообложения — ПСН / патент
```

Соответствие внутренним кодам Dreamkas API:
...
Важно: Dreamkas API для `/api/receipts` не принимает отдельный код `OSN`. Для общей системы используется `DEFAULT`. `ENVD` в этом методе не поддерживается.
...
В v6.34 исправлена ошибка, когда при выборе “Общая система налогообложения — ОСН” в Dreamkas отправлялся код:

```text
OSN
```

Dreamkas API возвращал ошибку валидации, потому что для `/api/receipts` допустимы только:
...
Теперь “общая система / ОСН” отправляется как:
...
## Отмена ввода реквизитов

В v6.34 добавлена отмена на этапе заполнения реквизитов нового чека.

Теперь можно вернуться в главное меню до создания предчека:
...
Также отмена доступна при вводе:
...
- типа покупателя;
- ИНН;
- наименования юрлица / ИП;
- ожидания заполнения Excel.
```

Для системы налогообложения `0` уже используется как `DEFAULT`, поэтому отмена там выполняется через:
